...
Linux の seg6local が 1 回の実行で消費する幅は `nflen` です。`lblen` は shift せずに残す locator block の長さで、SID の prefix 長は `lblen + nflen` になります。uA は node と function を同時に消費するので、F3216 では prefix が /64 になる `lblen 32 nflen 32` を指定します。`nflen 16` は uN の形で、function CSID が DA に残ります。

## 必要条件

- Linux kernel 6.6 以上。seg6local の End.X で next-csid flavor が使えることが必要です
- iproute2 6.0 以上
