...
ip netns exec "$ns_router1" ${VINBEROD_BIN} -c ${VINBERO_CONFIG} > /tmp/vinbero_headend_v6_test.log 2>&1 &
VINBERO_PID=$!
sleep 2
...
ip netns exec "$ns_router1" ${VINBERO_BIN} -s http://127.0.0.1:8082 hv6 create \
  --trigger-prefix 2001:2::/64 --src-addr fc00:1::1 --segments fc00:2::1,fc00:3::3 > /dev/null
