#!/bin/bash
# examples/end-am/setup.sh
# Setup End.AM (masquerading proxy, draft-ietf-spring-srv6-service-programming)
...
#
# Forward direction: router1 H.Encaps [fc00:2::150, fc00:3::3]. The End.AM
# SID fc00:2::150 on router2 masquerades the destination as the final
# segment and hands the packet — SRH still attached — to svc; svc forwards
# it back as a plain IPv6 router, and the return circuit de-masquerades
# from the in-packet SRH.

set -eu

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# Linux interface names are limited to 15 chars, so use short prefix "am-"
...
run ip netns exec "$ns_router2" ip addr add fc00:99::1/64 dev "$veth_rt2_svc"
run ip netns exec "$ns_svc" ip addr add fc00:99::2/64 dev "$veth_svc_rt2"

# The SR-unaware service: a plain IPv6 forwarder. It never looks at the
# SRH (the packets are not addressed to it) and routes everything back
# over the same wire.
ns_sysctl "$ns_svc" net.ipv6.conf.all.forwarding 1
run ip netns exec "$ns_svc" ip -6 route add fc00:3::/64 via fc00:99::1

print_info "Configuring SRv6 End.AM settings..."
...
run ip netns exec "$ns_router1" ip route add 172.0.2.0/24 encap seg6 mode encap segs fc00:2::150,fc00:3::3 dev "$veth_rt1_rt2"

# router2: Linux native End as the Phase 1 baseline (Linux cannot do
# End.AM; the baseline just proves the underlay path and resolves NDP).
...
run ip netns exec "$ns_router2" ip -6 route add local fc00:2::150/128 encap seg6local action End dev lo
...
echo ""
echo "=========================================="
echo "SRv6 End.AM Setup Complete!"
echo "=========================================="
echo "Forward: host1 -> [fc00:2::150 (End.AM via svc), fc00:3::3 (End.DX4)] -> host2"
echo "Return:  host2 -> [fc00:1::1 (End.DX4)] -> host1"
echo "Service namespace: $ns_svc (plain IPv6 forwarder on fc00:99::/64)"
echo ""
