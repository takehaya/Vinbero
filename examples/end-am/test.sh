#!/bin/bash
# examples/end-am/test.sh
# Test End.AM (masquerading proxy) with Vinbero XDP
...
echo "=========================================="
echo "SRv6 End.AM (masquerading proxy) Test"
echo "=========================================="
echo ""

# Phase 1: baseline without the proxy. Linux has no End.AM, so the proxy
# SID acts as a plain End; this proves the underlay chain and resolves
# the routers' neighbour entries.
echo "=========================================="
echo "Phase 1: Linux Native SRv6 (Baseline, End instead of End.AM)"
echo "=========================================="
...
ip netns exec "$ns_router2" ip -6 route del local fc00:2::150/128 2>/dev/null || true

echo ""

# Phase 2: Vinbero XDP End.AM - traffic must now detour through the
# service namespace and come back re-encapsulated.
echo "=========================================="
echo "Phase 2: Vinbero XDP End.AM"
echo "=========================================="

print_info "Starting Vinbero on $ns_router2..."
start_vinbero "$ns_router2" "${VINBERO_CONFIG}" "/tmp/vinbero_end_am_test.log"
...
print_info "Registering SidFunction (End.AM) entry (iface ${svc_ifindex}, service MAC ${svc_mac})..."
ip netns exec "$ns_router2" ${VINBERO_BIN} -s http://127.0.0.1:8082 sid create \
  --trigger-prefix fc00:2::150/128 --action END_AM \
  --inner-type ipv6 \
  --oif "$svc_ifindex" --iface-in "$svc_ifindex" \
  --service-mac "$svc_mac" > /dev/null

print_success "SidFunction (End.AM) entry registered"
sleep 1

rx_before=$(svc_rx_packets)

test_ping_with_counter "$ns_host1" 172.0.2.1 "host1 -> host2 (via End.AM proxy + service)"
...
if ip netns exec "$ns_router2" ${VINBERO_BIN} -s http://127.0.0.1:8082 sid create \
  --trigger-prefix fc00:2::151/128 --action END_AM \
  --inner-type ipv6 \
  --oif "$svc_ifindex" --iface-in "$svc_ifindex" \
