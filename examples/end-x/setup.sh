#!/bin/bash
# examples/end-x/setup.sh
# Setup End.X operation demonstration environment
...
# Configure SRv6-specific settings for End.X operation demo
print_info "Configuring SRv6 End.X operation settings..."
...
# Configure router2 (End.X) - Linux native for testing
# End.X forwards to an explicit nexthop instead of doing a standard FIB lookup
...
# Forward path: End.X with nexthop to router3 (fc00:23::1 is router3's address on rt2-rt3 link)
run ip netns exec "$ns_router2" ip -6 route add local fc00:2::1/128 encap seg6local action End.X nh6 fc00:23::1 dev "$veth_rt2_rt3"
# Return path: End.X with nexthop to router1 (fc00:12::1 is router1's address on rt1-rt2 link)
run ip netns exec "$ns_router2" ip -6 route add local fc00:2::2/128 encap seg6local action End.X nh6 fc00:12::1 dev "$veth_rt2_rt1"
...
echo ""
echo "=========================================="
echo "SRv6 End.X Setup Complete!"
echo "=========================================="
echo "Topology:"
echo "  host1 (172.0.1.1) <---> router1 (fc00:1::1, End.DX4)"
echo "  router1 (fc00:12::1) <---> router2 (fc00:2::1, fc00:2::2, End.X)"
...
echo ""
echo "End.X Nexthops:"
echo "  fc00:2::1 -> nh6 fc00:23::1 (router3 on rt2-rt3 link)"
echo "  fc00:2::2 -> nh6 fc00:12::1 (router1 on rt1-rt2 link)"
echo ""
