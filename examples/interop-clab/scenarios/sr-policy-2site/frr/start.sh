...
# --- SR Policy transport hop (End SID) -------------------------------------
# This scenario steers ce-osaka-bound traffic onto an SR Policy whose
# transport segment list is [fd00:200:0:ee::1]. That SID must terminate on
# this PE with an End behavior so the packet advances to the next segment
# (the End.DT4 service SID) and decaps into vrf-cust. FRR does not
# originate this localsid, so install it directly with seg6local: End
# decrements Segments Left, copies the next SID into the IPv6 DA, and
# re-runs the FIB lookup (which then hits the End.DT4 service SID).
# fd00:200:0:ee::1 sits in FRR's fd00:200::/48 block, so the core already
# routes it here.
ip -6 route replace fd00:200:0:ee::1/128 encap seg6local action End dev eth2
