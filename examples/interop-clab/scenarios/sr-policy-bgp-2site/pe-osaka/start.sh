#!/bin/bash
# Bring up the Vinbero PE pe-osaka for the sr-policy-bgp-2site scenario.
# The mirror of pe-tokyo: it advertises its own colored VPN route and an SR
# Policy for reaching itself, and steers its ce-tokyo-bound traffic through
# the waypoint using the policy it learns from pe-tokyo.
#
# Interfaces:
...
# pe-tokyo's loopback + locator block, plus the waypoint End block.
...
# Source locator: fd00:200::/48 is pe-osaka's SRv6 block.
...
# End.DT4 service SID -> vrf-cust -> ce-osaka.
/usr/local/bin/vbctl sid create \
    --trigger-prefix fd00:200:0:1::/128 \
    --action END_DT4 \
    --vrf-name vrf-cust || true

# Colored VPN route for the local customer subnet.
/usr/local/bin/vbctl bgp advertise-vpn --family vpnv4 \
    --prefix 10.2.0.0/24 --rd 65100:200 --rts 65000:100 \
    --sid fd00:200:0:1:: --next-hop 2001:db8:ff::2 --color 100 || true

# SR Policy for reaching THIS PE, transport via the waypoint End SID.
...
echo "[start.sh] pe-osaka (Vinbero) PE ready"
