...
# scenario.
#
# Hosts TWO uplink service instances on one node. Both access interfaces
# carry the same N3 addressing (172.16.0.254/24) and both VPNs use the same
# TEID, so the F-TEID tuple {endpoint, TEID} is identical across them — the
# uplink instances (vrf-bgp bind --mup-uplink-interfaces) are what keep the
# two F-TEID entries apart, keyed by the ingress ifindex.
#
...
# One uplink instance per VPN, created at runtime via the operator surface.
# The binding's mup_ipv4 import RT decides WHICH T2STs install under the
# instance, and mup-uplink-interfaces decides WHICH packets classify to it
# (by ingress ifindex). The vrf names are control-plane keys; no kernel VRF
# is needed on the GW.
/usr/local/bin/vbctl vrf-bgp bind --vrf vpn-a --rd 65100:1 \
    --rt mup_ipv4:100:6001:import --mup-uplink-interfaces eth1 || true
/usr/local/bin/vbctl vrf-bgp bind --vrf vpn-b --rd 65100:2 \
    --rt mup_ipv4:100:6002:import --mup-uplink-interfaces eth2 || true

# Pre-resolve the core next hop so bpf_fib_lookup on the redirect succeeds.
ping6 -c 1 -W 2 2001:db8:1::2 >/dev/null 2>&1 || true

echo "[start.sh] mup-gw (access MUP-GW) ready; uplink instances:"
