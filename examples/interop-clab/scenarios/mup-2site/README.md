...
resolution, since the controller's routes carry no SID — and both data
directions (tcpdump on DN for the decapped uplink inner packet, on gNB for the
downlink GTP-U). The uplink GTP-U is generated by a stdlib-only `send_gtpu.py`
(no scapy).
