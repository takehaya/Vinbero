#!/bin/bash
# examples/end-t-replace/setup.sh
...
# End.T(REP) plan (48-bit block fd00:aabb:ccdd, 32-bit C-SIDs, K=4):
#   r2: End.T(REP) C-SIDs b2b2:1 and b2b2:2  (/80, Vinbero, bound to vrf100)
#   r3: End(REP)   C-SID  b3b3:1             (/80, Vinbero)
#   r3: terminal   C-SID  b3b3:d             (/80, Linux End.DX4)
# Same ping-pong walk as end-replace (r2 -> r3 -> r2 -> r3), but both
# advances at r2 resolve the next C-SID in TABLE 100 ONLY: the main table
# carries a blackhole for the block, so a delivered ping proves both
# lookups used the bound VRF (End.T semantics) -- an End(REP) falling
# back to the default FIB would blackhole.
#
...
print_info "Configuring End.T(REP) settings..."
...
# router2: two End.T(REP) C-SIDs under Vinbero (registered in test.sh),
# both bound to vrf100. The block route towards r3 lives in TABLE 100
# only; the main table blackholes it, so any advance that resolved in
# the default FIB instead of the VRF would drop.
...
run ip netns exec "$ns_router2" ip -6 route add fd00:aabb:ccdd::/48 via fc00:23::1 dev "$veth_rt2_rt3" table 100
run ip netns exec "$ns_router2" ip -6 route add blackhole fd00:aabb:ccdd::/48
...
echo ""
echo "=========================================="
echo "SRv6 End.T(REP) Setup Complete!"
echo "=========================================="
echo "C-SID sequence (block fd00:aabb:ccdd/48, 32-bit C-SIDs):"
echo "  b2b2:1 (r2, vrf100) -> b3b3:1 (r3) -> b2b2:2 (r2, vrf100) -> b3b3:d (r3, End.DX4)"
echo "r2 main table: blackhole fd00:aabb:ccdd::/48 (VRF lookup structural proof)"
