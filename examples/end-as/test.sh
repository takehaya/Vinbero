#!/bin/bash
# examples/end-as/test.sh
# Test End.AS (static proxy) with Vinbero XDP
...
echo "=========================================="
echo "SRv6 End.AS (static proxy) Test"
echo "=========================================="
echo ""

# Phase 1: baseline without the proxy. Linux has no End.AS, so the proxy
# SID acts as a plain End; this proves the underlay chain and resolves
# the routers' neighbour entries.
echo "=========================================="
echo "Phase 1: Linux Native SRv6 (Baseline, End instead of End.AS)"
echo "=========================================="
...
# Phase 2: Vinbero XDP End.AS - traffic must now detour through the
# service namespace and come back re-encapsulated.
echo "=========================================="
echo "Phase 2: Vinbero XDP End.AS"
echo "=========================================="

print_info "Starting Vinbero on $ns_router2..."
start_vinbero "$ns_router2" "${VINBERO_CONFIG}" "/tmp/vinbero_end_as_test.log"
...
print_info "Registering SidFunction (End.AS) entry (iface ${svc_ifindex}, service MAC ${svc_mac})..."
ip netns exec "$ns_router2" ${VINBERO_BIN} -s http://127.0.0.1:8082 sid create \
  --trigger-prefix fc00:2::100/128 --action END_AS \
  --inner-type ipv4 \
  --oif "$svc_ifindex" --iface-in "$svc_ifindex" \
  --service-mac "$svc_mac" \
  --src-addr fc00:2::100 --segments fc00:3::3 > /dev/null

print_success "SidFunction (End.AS) entry registered"
sleep 1

rx_before=$(svc_rx_packets)

test_ping_with_counter "$ns_host1" 172.0.2.1 "host1 -> host2 (via End.AS proxy + service)"
...
if ip netns exec "$ns_router2" ${VINBERO_BIN} -s http://127.0.0.1:8082 sid create \
  --trigger-prefix fc00:2::101/128 --action END_AS \
  --inner-type ipv4 \
  --oif "$svc_ifindex" --iface-in "$svc_ifindex" \
  --service-mac "$svc_mac" \
  --src-addr fc00:2::101 --segments fc00:3::3 2>&1 | grep -qi "already bound"; then
