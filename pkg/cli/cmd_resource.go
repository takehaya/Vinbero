...
func vrfCommand() *cli.Command {
	// Kernel-device subcommands (create/delete/list) plus the ingress-facet
	// subcommands (ac-add/ac-remove/policy/show) under one `vrf` command, so the
	// kernel and ingress facets of a VRF share a single operator surface.
	sub := []*cli.Command{
		{
			Name:  "create",
	...
	sub = append(sub, vrfIngressSubcommands()...)
	return &cli.Command{
		Name:        "vrf",
