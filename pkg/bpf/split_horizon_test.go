...
func TestXDPProgEndDT2MSplitHorizonRX(t *testing.T) {
	...
	for _, tc := range tests {
		...
		SrcAddr:     peerSrc,
			...
			if err := h.mapOps.CreateBdPeer(bdID, 0, peerEntry, tc.peerEsi, peerEntry.SrcAddr); err != nil {
...
func TestXDPProgEndDT2MNonDFDrop(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := h.mapOps.CreateBdPeer(100, 0, peerEntry, senderESI, peerEntry.SrcAddr); err != nil {
				t.Fatalf("CreateBdPeer: %v", err)
			}
...
func TestXDPProgEndDT2SplitHorizonUnaffected(t *testing.T) {
	...
	if err := h.mapOps.CreateBdPeer(100, 0, peerEntry, esi, peerEntry.SrcAddr); err != nil {
		t.Fatalf("CreateBdPeer: %v", err)
	}
