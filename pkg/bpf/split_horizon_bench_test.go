...
func BenchmarkEndDT2MSplitHorizonDrop(b *testing.B) {
	...
	if err := h.mapOps.CreateBdPeer(benchBdID, 0, peer, esi, peer.SrcAddr, true); err != nil {
...
func BenchmarkEndDT2MForward(b *testing.B) {
	...
	peer, _ := benchL2Peer(b)
	if err := h.mapOps.CreateBdPeer(benchBdID, 0, peer, remoteESI, peer.SrcAddr, true); err != nil {
		b.Fatalf("CreateBdPeer: %v", err)
	}
