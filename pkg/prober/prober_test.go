...
func (f *fakeWire) send(t Target, token, seq uint16) error {
	f.sent = append(f.sent, sentProbe{t, token, seq})
	return nil
}
func (f *fakeWire) recv() (uint16, uint16, bool) { return 0, 0, false }
...
func replyAll(p *Prober, fw *fakeWire, from int, now time.Time) int {
	for _, s := range fw.sent[from:] {
		p.handleReply(s.token, s.seq, now)
	}
	return len(fw.sent)
}
...
func TestProber_StaleSeqIgnored(t *testing.T) {
	...
	// Straggler reply for round 1 lands during round 2.
	p.handleReply(first.token, first.seq, now.Add(150*time.Millisecond))
	p.mu.Lock()
	ps := p.tokens[first.token]
	seen := ps.replySeen
	p.mu.Unlock()
	if seen {
		t.Fatalf("a stale sequence marked the current round answered")
	}
}
...
func TestBuildEchoRequest_SRHLayout(t *testing.T) {
	...
	pkt, firstHop := buildEchoRequest(src, tgt, 0x1234, 7)
...
func TestBuildEchoRequest_NoSegments(t *testing.T) {
	src := netip.MustParseAddr("fd00::1")
	pkt, firstHop := buildEchoRequest(src, target(0, "fd00::d"), 1, 1)
