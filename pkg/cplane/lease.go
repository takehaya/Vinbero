// Package cplane holds the control-plane capability surface: the narrow,
// owner-scoped operations a plugin is granted, expressed so that Vinbero's
// own appliers could be written against the same shapes.
//
// Two ideas run through it. Writes are declared as a desired set per owner
// and reconciled by the core, rather than issued as an ordered stream of
// create and delete calls; and a key may only be written by one owner, so
// two writers racing for the same prefix is an error at the moment it is
// declared rather than a silent last-writer-wins.
package cplane
