...
func (f *fakeHeadend) CreateBdPeerAtFreeIndex(bdID uint16, e *bpf.HeadendEntry, esi [bpf.ESILen]byte, remoteSrc [bpf.IPv6AddrLen]byte, writeReverse bool) (uint16, error) {
	idx := f.FindFreeBdPeerIndex(bdID)
	if idx >= bpf.MaxBumNexthops {
		return idx, fmt.Errorf("bd %d: maximum number of peers (%d) reached", bdID, bpf.MaxBumNexthops)
	}
...
func (f *fakeHeadend) DeleteBdPeer(bdID, index uint16) (bool, error) {
	...
	_, existed := f.bdPeers[bdPeerKey{bdID, index}]
	if f.bdPeerDelErr != nil {
		return existed, f.bdPeerDelErr
	}
	if !existed {
		return false, nil
	}
...
// ListFdb renders the fake's FDB in the real map's key type, for the
// startup sweep of entries pointing into the ES range.
func (f *fakeHeadend) ListFdb() (map[bpf.FdbKey]*bpf.FdbEntry, error) {
	out := make(map[bpf.FdbKey]*bpf.FdbEntry, len(f.fdb))
	for k, v := range f.fdb {
		mac, err := net.ParseMAC(k.mac)
		if err != nil {
			return nil, err
		}
		fk := bpf.FdbKey{BdId: k.bdID}
		copy(fk.Mac[:], mac)
		out[fk] = v
	}
	return out, nil
