...
// An RT3 SID move whose teardown fails must abort the replacement: the
// old forward peer is still installed, and overwriting the kept ledger
// would leave it replicating BUM traffic with nothing tracking it.
func TestApplier_EVPNRT3SIDMoveAbortsOnDeleteError(t *testing.T) {
	a, fh := evpnApplier(t)
	a.Apply(bgp.RouteEvent{Family: bgp.FamilyEVPN, EVPN: rt3("fd00:2:2:24::")})
	...
	if len(fh.bdPeers) != 1 {
		t.Errorf("bd_peer count = %d, want 1 (no second peer installed)", len(fh.bdPeers))
	}

	// The retry (the same replacing RT3 after the map recovers) finishes
	// the move.
	fh.bdPeerDelErr = nil
	a.Apply(bgp.RouteEvent{Family: bgp.FamilyEVPN, EVPN: rt3("fd00:2:2:25::")})
