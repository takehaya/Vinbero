...
// End.DT2: Decapsulation with L2 table lookup (FDB)
// RFC 8986 Section 4.11
// Strips outer headers, looks up MAC in fdb_map for fast-path forwarding.
    ...
    struct sid_function_entry *entry)
{
    ...
    // 3. Strip outer headers (Eth + IPv6 + SRH)
    int strip_len = calc_decap_strip_len(srh);
    if (bpf_xdp_adjust_head(ctx, strip_len)) {
    ...
    // 4. Re-fetch pointers after adjust_head
    void *data = (void *)(long)ctx->data;
    void *data_end = (void *)(long)ctx->data_end;
    struct ethhdr *inner_eth = data;
    if ((void *)(inner_eth + 1) > data_end) {
    ...
    // 5. Build FDB lookup key (bd_id + MAC)
    struct fdb_key key = { .bd_id = entry->bd_id };
    __builtin_memcpy(key.mac, inner_eth->h_dest, ETH_ALEN);

    // 6. Lookup MAC in fdb_map
    struct fdb_entry *fdb = bpf_map_lookup_elem(&fdb_map, &key);
    if (!fdb) {
        // Unknown unicast: XDP_PASS to kernel bridge for flooding + learning
        DEBUG_PRINT("End.DT2: FDB miss, passing to kernel\n");
        return XDP_PASS;
    }

    // 7. Known unicast: fast-path redirect
    DEBUG_PRINT("End.DT2: FDB hit, redirect to ifindex %d\n", fdb->oif);
    STATS_INC(STATS_SRV6_END, 0);
