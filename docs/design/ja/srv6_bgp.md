...
uplink の状態はサービスインスタンス単位に分離できます。VRF binding に mup_uplink_interfaces で access interface を宣言すると、その binding は uplink instance を持ち、import RT が一致した T2ST の F-TEID エントリはその instance をキーに含めて install されます。data plane は ingress ifindex から instance を解決して lookup するので、複数のサービスインスタンスが同じ N3 endpoint アドレス空間と TEID 空間を使っても衝突しません。インスタンス間の fallback は無く、自分の instance に該当セッションが無い GTP-U はそのまま素通しになります。宣言の無い構成はすべて default instance 0 に落ちて、従来の挙動のままです。binding の変更は ifindex の対応表と install 済みセッションのキーを再 reconcile します。interop の検証構成は `examples/interop-clab/scenarios/mup-2site-multivrf` で、同一の {endpoint, TEID} を持つ 2 つの VPN が access interface だけで分離されることを assert します。
