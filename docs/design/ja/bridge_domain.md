...
現在、VRF・bridge・VLANの作成はLinux側（iproute2等）で事前に行い、VinberoのAPIでは名前やifindexで参照するだけの設計になっています。将来的にはVinberoがこれらのリソースもAPIで一括管理する方が、運用上の設定手順が簡素化される可能性があります。
...
# Linux bridge を作成する（Vinberoの管轄外）
ip link add br100 type bridge && ip link set br100 up
